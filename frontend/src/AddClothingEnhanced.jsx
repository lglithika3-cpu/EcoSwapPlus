...
export default function AddClothingEnhanced({ notify }) {
  ...
  const submit = async event => {
    ...
    try {
      ...
      setForm(initialForm); setFile(null); setPreview(''); notify('Item added to your active closet');
    } catch (submitError) { setError(submitError.message || 'Unable to publish listing'); }
  };
  return <><div className="page-title"><div><p className="eyebrow">Grow the circulation</p><h1>List a piece.</h1><p className="muted lead">Tell the story, set a fair value, and let it travel.</p></div></div><form className="form-shell" onSubmit={submit}><div className="form-grid">{[['title','Item title','text'],['brand','Brand','text'],['value','Estimated swap value','number'],['location','Location','text']].map(([name,label,type])=><label key={name}>{label}<input name={name} type={type} value={form[name]} onChange={update} required /></label>)}<label>Category<select name="category" value={form.category} onChange={update}>{['Tops','Dresses','Outerwear','Bottoms','Accessories'].map(value => <option key={value}>{value}</option>)}</select></label><label>Size<select name="size" value={form.size} onChange={update}>{['XS','S','M','L','XL','One size'].map(value => <option key={value}>{value}</option>)}</select></label><label>Condition<select name="condition" value={form.condition} onChange={update}>{['Like new','Excellent','Good','Well loved'].map(value => <option key={value}>{value}</option>)}</select></label><label className="full">Description<textarea name="description" value={form.description} onChange={update} required /></label><label className="upload full"><Upload size={22} />{file ? file.name : 'Photo upload'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { const selected=event.target.files?.[0] || null; setFile(selected); setPreview(selected ? URL.createObjectURL(selected) : ''); }} /></label>{preview && <div className="upload-preview full"><img src={preview} alt="Selected clothing preview" /><span>Ready to publish</span></div>}</div>{error && <p className="form-error">{error}</p>}<button className="button dark-button">Publish listing <ChevronRight size={17} /></button></form></>;
}
